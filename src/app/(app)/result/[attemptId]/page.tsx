...
export default async function ResultPage({
  ...
  const exam = EXAMS[attempt.examType];
      ...
      <div className="card p-6">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{formatDate(attempt.createdAt)}</p>
          </div>
          <span
            className={`chip ${
        ...
        <div className="mt-6 flex flex-col items-center gap-8 md:flex-row">
          <div className="flex flex-col items-center">
            <ScoreRing value={attempt.overall} headline={attempt.nativeHeadline} sub={exam.name} />
            <span className="mt-2 chip bg-brand-50 text-brand-700">{attempt.nativeLevel}</span>
            <span className="mt-1 text-xs text-slate-400">{exam.scaleLabel}</span>
          </div>
          <div className="flex-1">
            <DimensionCards scores={attempt.dimensions} />
            {r.summary && (
